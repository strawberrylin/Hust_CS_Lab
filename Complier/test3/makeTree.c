...
struct syntax_node *makeNode4(char *name,struct syntax_node *p1,struct syntax_node *p2,struct syntax_node *p3,struct syntax_node *p4){
    ...
    temp->r = p4;
    temp = temp->r;
    if(!strcmp(a->name,"CompSt")){
        a->scope = (struct scope *) malloc(sizeof(struct scope));
        a->scope->symbal_head = (struct symbal_node *)malloc(sizeof(struct symbal_node));
        a->scope->scope_kind = "block";
        a->scope->block = a;
    }
    return a;
}
